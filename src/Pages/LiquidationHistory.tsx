...
import { Datagrid, DateField, FilterForm, ListContextProvider, NumberField, Pagination, SearchInput, TextField, useList } from 'react-admin'
...
const fuseOptions = {
    ...
    // fieldNormWeight: 1,
    keys: ['chainId', 'tokenName', 'args.owner', 'args.buyer', 'args.collateralLiquidated', 'args.closingFee', 'args.debtRepaid'],
}
export default function LiquidationHistory(): JSX.Element {
    ...
    const filteredLiquidationLogs = !filter ? liquidationLogs : fuse.search(filter).map((r) => r.item)
    ...
    return (
        <ListContextProvider value={{ ...liquidationLogList, setFilters, filterValues: { q: filter } }}>
            <Grid container spacing={2}>
                ...
                <Grid xs={12}>
                    <Datagrid optimized>
                        ...
                        <OnChainHexField label="Owner" source="args.owner" addressType="address" />
                        <OnChainHexField label="Buyer" source="args.buyer" addressType="address" />
                        <NumberField label="Closing Fee" source="args.closingFee" />
                        <NumberField label="Collateral Liquidated" source="args.collateralLiquidated" />
                        <NumberField label="Debt Repaid" source="args.debtRepaid" />
                        <DateField label="Tx Timestamp" source="timestamp" />
                        <OnChainHexField label="TX" source="transactionHash" addressType="transaction" />
                    </Datagrid>
                    <Pagination rowsPerPageOptions={[10, 25, 50, 100]} defaultValue={50} />
                </Grid>
            </Grid>
        </ListContextProvider>
    )
