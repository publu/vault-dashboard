...
const EqiRewards: React.FC = () => {
    const { startOfPreviousPeriod, startOfCurrentPeriod } = generatePeriodTimestamps(new Date())

    const threeWeekOldBlockNumber = useBlockNumbersFromTS(previousSunday(startOfPreviousPeriod), ChainId.MATIC)?.data?.blocks?.[0]?.number

    const twoWeekOldBlockNumber = useBlockNumbersFromTS(startOfPreviousPeriod, ChainId.MATIC)?.data?.blocks?.[0]?.number

    const weekOldBlockNumber = useBlockNumbersFromTS(startOfCurrentPeriod, ChainId.MATIC)?.data?.blocks?.[0]?.number
    ...
    const onSubmit = async ({ distributionAmount, minimumEndBlock, snapshotBlock }: IEqiRewardForm) => {
