...
import { saveTemplateAsCsvFile, saveTemplateAsJsonFile } from '../../utils/files'

// const SUBGRAPH = "https://api.thegraph.com/subgraphs/name/publu/eqi-stuff";
const SUBGRAPH = 'https://api.thegraph.com/subgraphs/name/christianholman/eqi-subgraph'

export async function calculateRewards(snapshotBlock: BigNumber, minimumEndBlock: BigNumber, distributionAmount: BigNumber) {
    let first = 1000
    let skip = 0
    ...
    for (let i = 0; i < eligibleAccounts.length; i++) {
        ...
        const blockDifference = BigNumber.from(snapshotBlock).sub(BigNumber.from(account.blockNumber))
        const balance = eventBalance.sub(blockDifference.mul(BigNumber.from(account.rateOfDecay)))
        const newEqiBalance = {
            account,
    ...
    let shareSum: BigNumber = eqiBalances.reduce((a, b) => a.add(BigNumber.from(b.balance)), BigNumber.from('0'))
...
export function generateDisperse(
    ...
    const gnosisOutputLines = rewards.map((r) => {
        if (r.amount.gt(0)) {
            ...
            return `erc20,0x580A84C73811E1839F75d86d75d88cCa0c241fF4,${r.account},${formatUnits(r.amount.toString())}`
        } else {
            return null
        }
    })
