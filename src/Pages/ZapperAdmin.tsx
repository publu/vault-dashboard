...
const BeefyZapperForm: React.FC<{ zapper: BeefyZapper }> = ({ zapper }) => {
  ...
  const onSubmit: SubmitHandler<IBeefyZapperForm> = async (data) => {
    ...
    if (data.shouldAdd === "add")
      tx = await zapper.populateTransaction.addChainToWhiteList(
        data._asset,
        data._mooAsset,
        data._mooAssetVault
      );
    else
      tx = await zapper.populateTransaction.removeChainFromWhiteList(
        data._asset,
        data._mooAsset,
        data._mooAssetVault
      );
  ...
  if (chainId && provider) {
    switch (chainId) {
      case ChainId.MATIC:
        ...
        if (zapperAddress)
          zapper = CamZapper__factory.connect(zapperAddress, provider);
        break;
      case ChainId.FANTOM:
