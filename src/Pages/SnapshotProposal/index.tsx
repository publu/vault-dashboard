...
import { ChainId, COLLATERALS, GAUGE_VALID_COLLATERAL, GAUGE_VALID_COLLATERAL_V2, isGaugeValid } from '@qidao/sdk'
...
export default function SnapshotProposal() {
    const [snapshotTitle, setSnapshotTitle] = React.useState('QIP###: Vault Incentives Gauge (Round ##)')
...
`
    ...
    const choices = Object.values(COLLATERALS).flat().filter(isGaugeValid) as (GAUGE_VALID_COLLATERAL | GAUGE_VALID_COLLATERAL_V2)[]

    const [checked, setChecked] = useState<(GAUGE_VALID_COLLATERAL | GAUGE_VALID_COLLATERAL_V2)[]>(choices)
    const [startDateTime, setStartDateTime] = useState<Date | null>(new Date())
    const [endDateTime, setEndDateTime] = useState<Date | null>(addDays(3, new Date()))
    ...
    if (chainId && chainId !== ChainId.MATIC) return <div>Swap to Polygon, we need the correct block numbers</div>

    return (
        <Box sx={{ flexGrow: 1 }}>
            <Grid container spacing={0} style={{ minHeight: '100%' }}>
                <Grid xs={4}>
                    <CheckboxList choices={choices} checked={checked} setChecked={setChecked} />
                </Grid>
