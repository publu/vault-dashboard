...
const TOPIC0 = '0x4d151d3a98b83151d51917640c221f8c8e3c054422ea1b48dcbbd57e3f4210d5'
...
const chainHasScannerUrl = (chainId: ChainId | ChainIdWithScanner): chainId is ChainIdWithScanner => {
...
const scannerLimitQueues: { [k in ScannerUrl]: PQueue } = Object.fromEntries(
    Object.values(chainScannerApiUrlBase).map<[ScannerUrl, PQueue]>((scannerBaseUrl) => {
        return [scannerBaseUrl, new PQueue({ concurrency: 1, interval: 5100, intervalCap: 1 })]
    })
)
...
export interface LiquidationLog extends Omit<ethers.utils.LogDescription, 'args'> {
    args: {
        owner: string
        debtRepaid: number
        vaultID: number
...
export const useLiquidationHistory = (
    collaterals: (COLLATERAL | GAUGE_VALID_COLLATERAL | COLLATERAL_V2 | GAUGE_VALID_COLLATERAL_V2)[] | undefined
): UseQueryResult<LiquidationLog[] | null | undefined>[] => {
    return useQueries({
        queries: (collaterals || []).map((collateral) => {
            const chainId = collateral.chainId
            return {
                queryKey: [chainId, collateral?.vaultAddress || '', 'liquidation-history'],
                queryFn: async () => {
                    if (collateral) {
                        const vaultAddress = collateral.vaultAddress
                        const endDate = subMinutes(1, new Date())
                        const { data: endBlockData } = await getBlockNumbersFromTS(endDate, chainId)
                        const endBlock = endBlockData.blocks[0].number

                        const startDate = subMonths(2, endDate)
                        const { data: startBlockData } = await getBlockNumbersFromTS(startDate, chainId)
                        const startBlock = startBlockData.blocks[0].number

                        const liq = await fetchLiquidationHistory(chainId, startBlock, endBlock, vaultAddress)
                        ...
                        if (liq && liq.result && liq.result.map) {
                            const liquidationCalls: LiquidationLog[] = liq.result.map((foo) => {
                                ...
                                const collateralLiquidated = bigNumberToFloat(res.args.collateralLiquidated, collateral.token.decimals)
                                if (!buyer) {
                                    debugger
                                }
                                const id = [transactionHash, collateralLiquidated].join('-')
                                return {
                                    ...res,
                                    transactionHash: transactionHash,
                                    timestamp: fromUnixTime(BigNumber.from(foo.timeStamp).toNumber()),
                                    id,
                                    tokenName: collateral.token.name || '',
                                    chainId: collateral.chainId,
                                    args: {
                                        buyer: buyer,
                                        closingFee: bigNumberToFloat(res.args.closingFee, collateral.token.decimals),
