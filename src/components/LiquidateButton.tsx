...
const LiquidateButton: React.FC = () => {
  ...
  useEffect(() => {
    const generateButtonLabel = async (vaultChainId: ChainId) => {
      let provider = PROVIDERS[vaultChainId];
      if (account && metamaskProvider && provider) {
        let maiContract = MAIFACTORIES[vaultChainId].connect(
          maiAddresses[vaultChainName],
          provider
        );
        let allowance = await maiContract.allowance(
          account,
          vaultContract.address
        );
        console.log(
          `Allowance Check for ${vaultContract.address} on ${vaultChainName}`
        );
    ...
    <Button
      ...
      onClick={async () => {
        if (metaMaskIsActive && chainId && metamaskProvider && account) {
          ...
          let maiContract = MAIFACTORIES[vaultChainId as ChainId].connect(
            maiAddresses[vaultChainName],
            metamaskProvider
          );
          // let maiBalance = maiContract.balanceOf(account)
          let signerContract = maiContract?.connect(
            metamaskProvider.getSigner()
          );

          if (account && signerContract && chainId === vaultChainId) {
            let allowance = await signerContract.allowance(
              account,
              vaultContract.address
            );
            ...
            if (allowance <= vaultDebt) {
              ...
              let stablecoin = vaultFactory.connect(
                vaultContract.address,
                metamaskProvider
              );
              signerContract = stablecoin?.connect(
                metamaskProvider.getSigner()
              );
